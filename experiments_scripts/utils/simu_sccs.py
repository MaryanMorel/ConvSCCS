...
# This code has been forked from the Tick library: 
...
class SimuSCCS(Simu):
    ...
    hidden_features : `list`, default=[]
        Index of feature to be hidden after simulation to create omission bias
    ...
    def __init__(
        ...
        self.temporal_noise_level = temporal_noise_level
    ...
    def _simulate(self):
        ...
        if self.hidden_features:
            filter_features = lambda x: np.delete(x, self.hidden_features,
                                                  axis=1)
            features = [csr_matrix(filter_features(f.toarray())) for f in
                        features]
            censored_features = [csr_matrix(filter_features(f.toarray())) for f
                                 in censored_features]

        if self.temporal_noise_level > 0:
            features = [self._scramble_time(f, self.temporal_noise_level) for
                        f in features]
            censored_features = [self._scramble_time(f,
                                                     self.temporal_noise_level)
                                 for f in censored_features]
            outcomes = [self._scramble_time(csr_matrix(o),
                                            self.temporal_noise_level
                                            ).toarray().ravel()
                        for o in outcomes]
    ...
    def simulate_features(self, n_samples):
        ...
        if self.exposure_type == "single_exposure":
            features, n_samples = self._sim_hawkes_exposures(self.exposure_type)
        elif self.exposure_type == "multiple_exposures" and\
    ...
    def _sim_hawkes_exposures(self, exposure_type='single_exposure'):
        if not self.sparse:
            raise ValueError(
        ...
        if exposure_type == 'single_exposure':
            timestamps = [[
                np.array([np.min(np.floor(f))]) if len(f) > 0 else np.array([])
                for f in patient_events]
                for patient_events in hawkes.timestamps]
        elif exposure_type == 'multiple_exposures':
            timestamps = hawkes.timestamps
        else:
            raise ValueError("exposure_type should be either 'single_exposure'"
                             "or 'multiple_exposures'.")

        # features = [
        #     self.to_coo(feat, (run_time, self.n_features)) for feat in features
        # ]
        features = self.to_coo(timestamps, (run_time, self.n_features))
    ...
    @staticmethod
    def to_coo(timestamps_samples, shape):
        # TODO: make this work for multiple_exposures
        coordinates = [(np.ones_like(np.hstack(timestamps)),
                        (np.hstack([np.floor(t) for t in timestamps]).astype('int'),  # rows
                        np.hstack([np.repeat(i, len(t)) for i, t in
                                   enumerate(timestamps)])  # cols
                        ))
                for timestamps in timestamps_samples]
        feat = [csr_matrix(c, shape=shape) for c in
                coordinates]
        # feat = np.array(feat)
    ...
    @staticmethod
    def _scramble_time(csr_mat, noise_upper_bound):
        """Add noise in the temporal pattern described by longitudinal feature
        matrices."""
        coo_mat = csr_mat.tocoo()
        new_row = coo_mat.row
        noise = np.random.randint(0, noise_upper_bound, new_row.shape)
        noise_direction = (np.random.random(new_row.shape) > 0.5) * 2 - 1
        new_row = new_row - noise * noise_direction
        new_row[new_row < 0] = 0
        new_row[new_row >= coo_mat.shape[0]] = coo_mat.shape[0] - 1
        return csr_matrix((coo_mat.data, (new_row, coo_mat.col)),
                          shape=csr_mat.shape)
